...
const stripe = new Stripe('pk_live_51Oh9oSHA4IKR1tD0kYQmr5POeEWaQbZpOOUvecFZgL55PeNR7OiAfRtRqEs3cp7qzcxhpFyWv0n722SZLhGECH3O00u0LrmckE');
...
if (process.client) {
    ...
    if (res) {
        ...
        if (res.value.subscription === undefined) {
            color.value = 'vermelho'
        } else if (res.value.subscription.status === 'active') {
            res.value.subscription.status = 'ativo'
            color.value = 'verde_claro'
        } else if (res.value.subscription.status === 'trialing') {
            res.value.subscription.status = 'Período de teste'
            color.value = 'verde'
        } else if (res.value.subscription.status === 'paused') {
            res.value.subscription.status = 'expirada'
            color.value = 'vermelho'
        }
        else {
            color.value = 'laranja'
...
<template>
    <div v-if="screen === 'desktop'">
        ...
        res.subscription ? res.subscription.status : 'Inexistente'
    }}</span></p> 
                    <p class="text-escuro ">Expira em: <span class="font-bold">{{ res.dataExpiracao }}</span></p>
                </div>
                <img class="h-[70px]" src="../assets/icons/saffron.svg" alt="">
            </div>
            <h1 v-if="res.subscription === undefined" class=" text-vermelho font-bold animate-pulse">Sua Assinatura não
                está
                ativa!
            </h1>
            <h1 v-else-if="res.subscription.status === 'expirada'" class=" text-vermelho font-bold animate-pulse">Sua
                Assinatura
                não está
                ativa!
            </h1>
            <NuxtLink v-if="!res.subscription" to="https://saffron.com.br/comprar"
            ...
            <NuxtLink v-else-if="res.subscription.status === 'expirada'" :to="`${res.session.url}`"
                class="bg-vermelho text-white font-semibold text-center cursor-pointer  border-l-8flex justify-evenly p-4 max-w-[30vw] mb-5">
                Clique aqui para obter sua assinatura
            </NuxtLink>
            <div class="bg-verde_apagado  border-l-8 border-l-verde flex-row p-4 max-w-[30vw]">
                ...
                <div class="flex w-full justify-evenly space-x-4">
                    ...
                    <NuxtLink v-if="res.subscription != ''" :to="`${res.session.url}`" data-modal-toggle="defaultModal"
                        type="button"
    ...
    <div v-if="screen === 'mobile'">
        ...
        <div class=" flex flex-col justify-center">
            <div
                class="bg-verde_apagado text-escuro border-l-8 border-l-verde flex justify-evenly items-center p-4  mb-5 ">
                <div>

                    <p class='text-escuro '>Assinatura: <span :class="`text-${color} font-semibold capitalize`">{{
        res.subscription ? res.subscription.status : 'Inexistente'
    }}</span></p>
                    <p v-if="res.subscription != undefined" class="text-escuro ">Expira em: <span class="font-bold">{{
        res.dataExpiracao }}</span></p>
                </div>
            ...
            <h1 v-if="!res.subscription" class=" text-vermelho font-bold animate-pulse">Sua Assinatura não está
                ativa!
            </h1>
            <h1 v-else-if="res.subscription.status === 'expirada'" class=" text-vermelho font-bold animate-pulse">Sua
                Assinatura
            ...
            <div
                class="bg-verde_apagado  text-escuro border-l-8 border-l-verde flex-row items-center justify-center p-4 ">
                <h1 class="text-escuro font-semibold text-xl mb-6">Informações da conta</h1>
                ...
                <NuxtLink v-if="res.subscription != ''" @click="handleSubmitSetup" :to="`${res.session.url}`"
